  ...
  draw() {
   ...
   // console.log("draw - Endboss-Status", this.endboss.status);
    if (this.endboss.status === true) {
        this.ctx.filter = 'brightness(50%)'; // Hintergrund abdunkeln
    } else {
...
handleEndboss(){
  ...
  if (this.isPepeNearEndboss() < 300) {
    ...
    console.log(
      "draw / isPepeNearEndboss: EndbossClose",
      this.endboss.EndBossClose
    );
  } else {
...
console.log("3. Anzahl der Flaschen nach dem Wurf:", this.collectedBottles);
    ...
    } else if (keyboard.THROW && this.collectedBottles === 0) {
      this.canThrow = false;

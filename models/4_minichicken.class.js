class MiniChicken extends MovableObject{
    ...
    offset = {
        ...
        constructor(world) {
            ...
            this.speed = 3.2 + Math.random() * 1.0; // Zufällige Geschwindigkeit zwischen 0.1 und 0.6
            this.animationSpeed = 100 + Math.random() * 300; // Zufällige Animationsgeschwindigkeit zwischen 100 und 300 ms
            this.type = 'minichicken';
            this.visible_width = 800; 
            // this.camera_x = this.world.cameraX; // Canvas-Breite
            this.animateX();
            this.animateWalk();
...
animateX() {  
    // this.otherDirection = true; // Startet mit Linkslauf
    let movingRight = false; // Startet nach links

    this.animateXInterval = setInterval(() => { 
        // Toggle Richtung basierend auf Position
...
// animateBounce() {
//     if (this.animateBounceActive) return; // Verhindert mehrfache Bounces
//     this.playingBounceAudio = false;
//     this.animateBounceActive = true;
//     let defaultSpeed = this.speed;
//     this.speed = 0;
    
//     // Stoppe Walk-Animation
//     clearInterval(this.animateWalkInterval);
    
//     // Starte Bounce-Animation
//     this.animateBounceMiniInterval = setInterval(() => {
//         this.playingBounceAudio = true;
//         this.playAnimation(MiniChicken.IMAGES_HIT);
//          this.audio.playEffect('mini_bounce', { play: true, pause: false , loop:false});
//             this.audio.playAudio(this.soundName, { play: false, pause: true });
//     }, this.animationSpeed); 
//     // this.playingBounceAudio = false;
...
//          this.audio.controlAudio(this.soundName, { play: true, pause: false, loop:true});
//         this.animateWalk();
//     }, 3000);
// }
...
setupSoundInterval() {
    // Zufälliger Startzeitpunkt für jedes Chicken
    setTimeout(() => {
        setInterval(() => {
            if (this.animateBounceActive) return;
            ...
            if (volume > 0) {
                this.audio.controlAudio(this.soundName, {volume: volume * 0.3, loop: false});
            }
        }, 200); //2000 + Math.random() * 3000);
    }, 300); // Verzögerter Start zwischen 0-1000ms
}
...
isVisible() {
    ...
    return this.x + this.width >= -camera_x && 
...
calculateVolume() {
    ...
    // Lautstärke zwischen 0 und 1, je näher zur Mitte desto lauter
